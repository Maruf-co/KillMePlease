/**
 * The SymbolSmallS class extends abstract class Symbol, implements
...
public class SymbolSmallS extends Symbol implements Passive, SmallCase {
     ...
     * SymbolSmallS by given paramters
    ...
    @Override
    public void upgrade() {
        Symbol tmp = new SymbolCapitalS(idSymbol, position); // Create new symbol of type SymbolCapitalP with id of current symbol
        ...
        for (int i = 1; i <= sightDistance; i++) { // Going through sight distance
            if (position.row + i < 10 && position.row != 0){ // Checking for moving symbol Up
                ...
                if (WorldController.world.containsKey(tmpPosition) && // Checking whether key exist
                        ...
                        !(WorldController.world.get(tmpPosition).get(0) instanceof SymbolSmallS)){ // Checking whether symbol of other type
    ...
    public void moveBreed() {
        ...
        for (int i = 1; i <= sightDistance; i++) {
            if (position.column + i < 10){ // Checking for moving symbol Down
                ...
                if (WorldController.world.containsKey(tmpPosition) && // Checking whether key exist
                        WorldController.world.get(tmpPosition).size() != 0 && // Checking whether list is not empty
                        WorldController.world.get(tmpPosition).get(0) instanceof SymbolSmallS){ // Checking whether symbol of same type
